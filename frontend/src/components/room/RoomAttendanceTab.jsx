...
import { getAttendanceRecords, updateAttendanceRecord, deleteAttendanceRecord } from '../../api';
...
export default function RoomAttendanceTab({ roomId, initialStatus = 'ALL' }) {
  const { user } = useAuth();
  ...
  const loadRecords = async () => {
    setLoading(true);
    try {
      const aRes = await getAttendanceRecords({
        page,
        page_size: pageSize,
      ...
      setRecords(aRes.data.items);
      setTotal(aRes.data.total);
      setTotalPages(aRes.data.total_pages);
    } catch (e) {
  ...
  useEffect(() => { setPage(1); }, [debouncedSearch, filterStatus, roomId]);
  useEffect(() => { loadRecords(); }, [page, pageSize, debouncedSearch, filterStatus, roomId]);
  ...
  const handleDelete = (rec) => {
    setConfirm({
      ...
      message: `Xóa bản ghi của "${rec.full_name}"?`,
      confirmLabel: 'Xóa',
      onConfirm: async () => {
        try {
          ...
          <div className="card w-full max-w-lg p-6 shadow-[var(--shadow-lg)] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {viewingRec && (
              <div className="mb-4 p-3 rounded-[var(--radius-sm)] bg-[var(--accent-bg)] text-xs space-y-1">
                <p className="m-0"><strong>{viewingRec.full_name}</strong></p>
